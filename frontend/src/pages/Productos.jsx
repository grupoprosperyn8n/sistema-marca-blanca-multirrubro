...
export default function Productos() {
  ...
  const filtrados = categoriaActiva === "Todos"
    ? productos
    : productos.filter(p => formatCategoria(p.categoria_publica) === categoriaActiva);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-12">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-base max-w-lg mx-auto" style={{ color: "var(--brand-text-secondary)", lineHeight: 1.7 }}>
          {config.productsSubtitle || "Productos profesionales seleccionados para el cuidado de tu belleza"}
        </p>
      </div>
      ...
      {filtrados.length === 0 ? (
        <div className="glass-panel p-12 text-center rounded-2xl">
          ...
          <h3 className="text-xl font-semibold mb-2" style={{ color: "var(--brand-text)" }}>Próximamente</h3>
          <p className="text-sm opacity-50" style={{ color: "var(--brand-text-secondary)" }}>
            Estamos preparando nuestro catálogo de productos. ¡Volvé pronto!
          </p>
        </div>
      ) : (
